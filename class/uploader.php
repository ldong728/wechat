...
class uploader
{
     ...
     */
    public function __construct( $weixinId,$fileName, $fileField , $config , $base64 = false )
    {
     ...
     * @param $md5 上传文件的md5值
     */
    private function checkFileMd5($news_id,$md5){
        $query=pdoQuery('img_tbl',null,array('md5'=>$md5),'limit 1');
//        wxlog('query');
        if($row=$query->fetch()){
            wxlog('match');
...
//        wxlog('nomatch');
    ...
    private function base64ToImage( $base64Data )
    {
        ...
        $this->fileName = $this->fileName.time(). rand( 1 , 10000 ).".png";
    ...
    {
        return $this->fileName = $this->fileName.time(). rand( 1 , 10000 ). $this->getFileExt() ;
